...
- If your game crashes, a dialog will open asking if you want to bisect else (todo) click the start bisect button at the top of your screen and you will prompted to give your issue a name
- The game will automatically restart. Try to reproduce the issues you had before.
  - (todo) If everything is fine, click the `Fine, continue bisect` button
  - (todo) if you have an issue click the `Continue bisect with issue` button, and select the issue you are having or create a new one
...
If minecraft didnt quit normally (either through a crash or through the bisect menu) and a bisect is active, the parent process will restart the game.
...
- if all sections only contain one element, an minimal reproduction has been isolated. Mark the corresponding issue as fixed with the resulting mod set and debug the next issue, with one of the current mods removed
...
Instead, Quilt Bisect simply ignores dependency in it's search algorithm and provides all mods to quilt loader.
